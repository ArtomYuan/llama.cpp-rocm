...
// ---------------------------------------------------------------------------------------------
    ...
    // ROCmFP4: same 32-value/QI=4 tiling as MXFP4. The dual-scale variant needs the
    // Q3_K scale space (2 scales per block), the single-scale FAST variant does not.
    ...
    // ROCmFPX (fp8 family), ported from charlie12345/ROCmFPX (extraction method).
    // fp2/fp3/fp6 use the Q3_K SRAM layout (dual finite-UE4M3 scales per 32-value
    // block); fp8 uses the Q8_0 layout (single finite-UE4M3 scale per block).
    // Wired for RDNA3.5 only for now (see ggml_cuda_should_use_mmq).
