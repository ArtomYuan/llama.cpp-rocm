...
// --- ROCmFP4 RDNA3.5 launch tuning (from charlie12345/ROCmFPX) -----------------
// Strix Halo previously inherited the RDNA2 one-warp MMVQ table for these
// layouts. The two-warp route below is that project's measured default; the
// knobs are kept so the geometry can be re-measured rather than assumed.
